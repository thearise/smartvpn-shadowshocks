...
Polymer({
  _template: html`
    <style>
      ...
      .init-div server-connection-indicator {
        width: 192px;
        height: 192px;
      }
    </style>
    <div class="server-list-container" style="padding-top: 5px;">
      <template is="dom-if" if="[[shouldShowZeroState]]">
        <div class="flex-column-container">
          <div class="flex-column-container">
            <div class="init-div" style="align-items: center !important;">
              <server-connection-indicator connection-state="disconnected"></server-connection-indicator>
              <div class="header">At first launch, internet connection required</div>
              <div class="subtle">စတင်ရန်အတွက် အင်တာနက်လိုသည်</div>
            </div>
          </div>
      ...
      <template is="dom-if" if="[[!shouldShowZeroState]]">
        <server-list
          id="serverList"
          servers="[[servers]]"
          localize="[[localize]]" style="padding-left: 15px; padding-right: 15px;"
        ></server-list>
      </template>
    </div>
  `,
