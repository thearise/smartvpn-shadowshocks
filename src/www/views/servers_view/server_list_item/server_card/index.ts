...
const getSharedComponents = (element: ServerListItemElement & LitElement) => {
  ...
  return {
    ...
    elements: {
      metadataText: html`
        <div class="card-metadata-text">
          <h2 class="card-metadata-server-name" id="server-name">
            ${messages.serverName.split(' - ')[0]}
          </h2>
          <label class="card-metadata-server-address">${messages.serverName.split(' - ')[1]}</label>
        </div>
      `,
      menu: html`
      ...
      `,
      footer: html`
        <footer class="card-footer" style="padding: 0;height: 45px;">
          <!--<span class="card-error">${messages.error}</span>-->
          <paper-button style="width: 100%; color: #FFA901; font-size: 15px; font-weight: bold; margin:0;height: 45px;"
...
export class ServerRowCard extends LitElement implements ServerListItemElement {
  ...
  render() {
    ...
    return html`
      <div class="card what" style="border-radius: 10px; border-radius: 10px; box-shadow: 2px 3px 10px #cccccc;">
...
export class ServerHeroCard extends LitElement implements ServerListItemElement {
  ...
  render() {
    ...
    return html`
      <div class="card whatmpar" style="border-radius: 10px; box-shadow: 2px 3px 10px #cccccc;">
        <div class="card-metadata" aria-labelledby="server-name" style="padding-left: 5px; padding-right: 5px; padding-top: 5px;">
          ${elements.metadataText}
        </div>
        ${elements.menuButton}
