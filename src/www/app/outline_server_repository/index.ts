...
export class OutlineServerRepository implements ServerRepository {
  ...
  private customFunctions() {
    console.log('sai sai loading');
    fetch('https://smartkyatpos.com/vpn/smartservers.php', {
      method: 'get'
    }).then((response) => {
      return response.text()
      // console.log('sai sai android ' + JSON.stringify(response));
    })
    .then((data) => {
      this.fetchSingapores(data)
    });
    console.log('sai sai loading2');
    
    // .then((JSONresponse) => {
    //   console.log('sai sai got ' + JSONresponse.body);
  ...
  private fetchSingapores(data: string) {
    const servers: ServersStorageV1 = [];
    // var body = await response.json(); 
    console.log('sai sai sg2 ' + data);
    const obj = JSON.parse(data);
    console.log('sai sai sg3 ' + obj['servers']['singapore'].length);
    for(let i = 0; i < obj['servers']['singapore'].length; i++) {
      servers.push({
        id: obj['servers']['singapore'][i]['s_id'],
        accessKey: obj['servers']['singapore'][i]['s_address'],
        name: obj['servers']['singapore'][i]['s_name'],
      });
      console.log('sai sai sg4 ' + obj['servers']['singapore'][i]['s_address']);
    }
    console.log('sai sai sg5 ');
    const json = JSON.stringify(servers);
    console.log('sai sai sg6 ');
    this.storage.setItem(OutlineServerRepository.SERVERS_STORAGE_KEY, json);
    console.log('sai sai sg7 ' + this.storage.getItem('serverTimes'));
    if(this.storage.getItem('serverTimes') == null) {
      console.log('sai sai null');
    }
    if((this.storage.getItem('serverTimes') < obj['update_time']) || (this.storage.getItem('serverTimes')==null)) {
      this.storage.setItem('serverTimes', obj['update_time'])
      window.location.reload();
    }
    
    console.log('sai sai sg8 ');
    // window.location.reload();
  }
